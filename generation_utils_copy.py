...
def preprocess(
    ...
    **kwargs,
    ...
    if do_resize:
        ...
        '''
        # print(images[0].dtype) #we use uint8 this should solve the problem with bicubic --> https://github.com/pytorch/vision/issues/2950#issuecomment-2285840770
        images = [T.Resize((size['height'], size['width']), interpolation=InterpolationMode.BICUBIC, antialias=True)(image) for image in images]
        # images = [image.clip(0,1) for image in images]
    ...
    if do_normalize:
        ...
        # transforms.append(T.Lambda(lambda img: (img - image_mean_tensor) / image_std_tensor))
