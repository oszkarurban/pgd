...
def pgd(captioning_model, captioning_processor, x, target, k, eps, eps_step, target_text, clip_min, clip_max):
    ...
    input_ = x_adv.clone().detach().to("cuda")
    ...
    optimizer = Adam([input_])

    pbar = trange(k, desc="Loss - Pred", leave=True)
    curr_time = str(int(np.round(time.time() * 1000)))

    for i in pbar:
        # input_ = input_.detach().clone() -if this line is thre the loss doesnt go down
        ...
        if i % 20 == 0:
            ...
            plt.ylabel('Loss')
            plt.title(f'PGD Attack - target: {target_text}')
            plt.legend()

            # Save the plot
            save_path = "/home/ubuntu/fromlocalethcomputer/images/visualizeimages"
            plt.savefig(os.path.join(save_path, f"pgd_attack_loss_{curr_time}.png"))
            plt.close()
...
if __name__ == "__main__":
    ...
    #LOAD IMAGE
    image = Image.open("./examples/before_pgd_Set_of_Pfaltzgraff_.png")
    ...
    adv = pgd(captioning_model,captioning_processor, image, target=input_ids, k=100, eps=0.2, eps_step=0.3, target_text=target_text[0], clip_min=0, clip_max=1.0)
    

    #SAVE
    path="./visualizeimages/"+"after_pgd.png"
